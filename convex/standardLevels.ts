...
export const list = query({
  ...
  handler: async (ctx) => {
    ...
    const nextIdSet = new Set(
      levels.map((l) => l.nextLevelId).filter(Boolean)
    );
    const root = levels.find((l) => !nextIdSet.has(l._id) && levels.some(x => x.nextLevelId === l._id || !levels.find(x2 => x2.nextLevelId === l._id)));

    // Fallback: sort by code if linked list walk fails
    const codeOrder = ["PG","NUR","KG1","KG2","01","02","03","04","05","06","07","08","09","10","11","12"];
    return levels.sort(
